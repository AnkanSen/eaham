...
app.get("/isRegisteredOnEvent",async(req,res)=>{
  ...
  const currEventName=req.query.currEventName;
  const eventData=await Event.find({eventName:currEventName})
...
app.get("/allParticipants", async (req, res) => {
  const eventName = req.query.currEventName;
  eventData = await Event.find({ eventName: eventName });
